...
class CancelPdfPage extends StatelessWidget {
  ...
  Future<Uint8List> _generatePdf() async {
    ...
    final userId = cancelData['user_id'];
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          _infoTable([
            if ((bookingData['booking_id']?.toString() ?? '').isNotEmpty)
...
// CANCELLATION INFORMATION
          ...
          _infoTable([
            if ((cancelData['reason'] ?? '').toString().isNotEmpty) ['REASON', cancelData['reason']],
            ['CANCELLED ON', DateFormat('dd-MM-yyyy hh:mm a').format(
                cancelData['created_at'] != null ? DateTime.parse(cancelData['created_at']).toLocal() : DateTime.now())],
          ], lightBlue, font),
          pw.SizedBox(height: 10),
          pw.Row(
              ...
              pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    // Cancel Charge
                    if ((cancelData['cancel_charge'] ?? '').toString().isNotEmpty)
                      pw.Container(
                        width: double.infinity,
                        color: lightBlue,
                        padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 4),
                    ...
                    // Refund
                    if ((cancelData['refund'] ?? '').toString().isNotEmpty)
                      pw.Container(
                        width: double.infinity,
                        color: lightBlue,
                        padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 4),
