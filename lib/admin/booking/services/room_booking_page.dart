...
class _RoomBookingPageState extends State<RoomBookingPage> {
  ...
  Widget bookingDetailsCard({
    ...
    String formatTo12Hour(DateTime dt) {
      return "${dt.day.toString().padLeft(2, '0')}-"
          ...
          "${dt.year} "
          "${(dt.hour % 12 == 0 ? 12 : dt.hour % 12).toString().padLeft(
          2, '0')}:"
    ...
    // Parse alternate phone (JSON string or comma-separated)
    List<String> alternatePhones = [];
    if (booking['alternate_phone'] != null && booking['alternate_phone']
        .toString()
        .isNotEmpty) {
      try {
        ...
        alternatePhones =
            booking['alternate_phone'].toString().split(',').map((e) =>
                e.trim()).toList();
      }
    }

    Text buildLabelValue(String label, String value) =>
        Text(
          "$label: $value",
          style: TextStyle(fontSize: 15, color: royal),
        );

    String generateShareText() {
      final buffer = StringBuffer();
      buffer.writeln("Booking ID: ${booking['booking_id'] ?? 'N/A'}");
      if (booking['name'] != null) buffer.writeln("Name: ${booking['name']}");
      if (booking['phone'] != null) buffer.writeln("Phone: ${booking['phone']}");
      if (alternatePhones.isNotEmpty) buffer.writeln("Alternate Phone(s): ${alternatePhones.join(', ')}");
      if (booking['email'] != null && booking['email'].toString().isNotEmpty) buffer.writeln("Email: ${booking['email']}");
      if (booking['address'] != null && booking['address'].toString().isNotEmpty) buffer.writeln("Address: ${booking['address']}");
      if (booking['check_in'] != null) buffer.writeln("Check-in: ${formatDateTime(booking['check_in'])}");
      if (booking['check_out'] != null) buffer.writeln("Check-out: ${formatDateTime(booking['check_out'])}");
      if (booking['numberofguest'] != null) buffer.writeln("Guests: ${booking['numberofguest']}");
      if (roomNumbers != null && roomNumbers.isNotEmpty) buffer.writeln("Rooms: ${roomNumbers.join(', ')}");
      if (booking['room_name'] != null) buffer.writeln("Room Name: ${booking['room_name']}");
      if (booking['room_type'] != null) buffer.writeln("Room Type: ${booking['room_type']}");
      if (booking['baseamount'] != null) buffer.writeln("Base Amount: ${booking['baseamount']}");
      if (booking['gst'] != null) buffer.writeln("GST: ${booking['gst']}");
      if (booking['amount'] != null) buffer.writeln("Total Amount: ${booking['amount']}");
      if (booking['advance'] != null) buffer.writeln("Amount Paid: ${booking['advance']}");
      if (booking['Balance'] != null) buffer.writeln("Balance: ${booking['Balance']}");
      if (idProofs != null && idProofs.isNotEmpty) buffer.writeln("ID Proofs: ${idProofs.join(', ')}");
          ...
          ConstrainedBox(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  const SizedBox(height: 20), // Space for floating header

                  // Personal Info
                  if (booking['name'] != null || booking['phone'] != null)
                    Column(
                      children: [
                        if (booking['name'] != null) buildLabelValue(
                            "Name", booking['name']),
                        if (booking['phone'] != null) buildLabelValue(
                            "Phone", booking['phone']),
                        if (alternatePhones.isNotEmpty)
                          buildLabelValue(
                              "Alternate Phone(s)", alternatePhones.join(', ')),
                        if (booking['email'] != null && booking['email']
                            .toString()
                            .isNotEmpty)
                          buildLabelValue("Email", booking['email']),
                        if (booking['address'] != null && booking['address']
                            .toString()
                            .isNotEmpty)
                    ...
                    Column(
                      children: [
                        ...
                        if (booking['check_out'] != null)
                          buildLabelValue("Check-out",
                              formatDateTime(booking['check_out'])),
                        if (booking['numberofguest'] != null)
                          buildLabelValue("Number of Guests",
                              booking['numberofguest'].toString()),
                        if (roomNumbers != null && roomNumbers.isNotEmpty)
                          buildLabelValue(
                              "Rooms Booked", roomNumbers.join(', ')),
                        if (booking['room_name'] != null)
                          buildLabelValue("Room Name", booking['room_name']),
                        if (booking['room_type'] != null)
                          buildLabelValue("Room Type", booking['room_type']),
                      ],
                    ),
                  ...
                  Column(
                    children: [
                      if (booking['baseamount'] != null)
                        buildLabelValue(
                            "Base Amount", booking['baseamount'].toString()),
                      if (booking['gst'] != null)
                        buildLabelValue("GST", booking['gst'].toString()),
                      if (booking['amount'] != null)
                        buildLabelValue(
                            "Total Amount", booking['amount'].toString()),
                      if (booking['advance'] != null)
                        buildLabelValue(
                            "Amount Paid", booking['advance'].toString()),
  ...
  Future<void> _bookRooms() async {
    ...
    for (int i = 0; i < numGuests; i++) {
      if (guestIdProofs[i] == null) {
        _showMessage("Upload ID proof for guest ${i + 1}");
        return;
      }
    }
    ...
    for (int i = 0; i < guestIdProofs.length; i++) {
      if (guestIdProofs[i] != null) {
        request.files.add(
          await http.MultipartFile.fromPath(
            'id_proofs',
            guestIdProofs[i]!.path,
          ),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      appBar: AppBar(
        backgroundColor: royal,
        title: const Text("Lodge Booking", style: TextStyle(color: Colors.white)),
