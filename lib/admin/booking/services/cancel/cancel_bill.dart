...
class CancelBillPage extends StatelessWidget {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          _row("Name", bookingDetails['name'], greyBox),
          ...
          _row("Total Amount", "₹${bookingDetails['amount']}", greyBox),
          _row("Advance Paid", "₹${bookingDetails['advance']}", greyBox),
          _row("Balance", "₹${bookingDetails['Balance']}", greyBox),
          pw.SizedBox(height: 20),

          // 🔵 Cancel section (from serverData)
          pw.Text("Cancellation Details",
              style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 6),

          _row("Cancelled By", serverData?['user_id'].toString() ?? "-", greyBox),
          _row("Reason", serverData?['reason'] ?? "-", greyBox),
          _row("Amount Paid", "₹${serverData?['amount_paid'] ?? 0}", greyBox),
          _row("Cancel Charge", "₹${serverData?['cancel_charge'] ?? 0}", greyBox),
          _row("Refund Amount", "₹${serverData?['refund'] ?? 0}", greyBox),
          _row("Cancelled At", serverData?['created_at']?.toString() ?? "-", greyBox),
        ],
      ),
    );
