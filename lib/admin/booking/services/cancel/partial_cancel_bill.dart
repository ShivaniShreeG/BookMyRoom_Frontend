...
class PartialCancelBillPage extends StatefulWidget {
  ...
  @override
  State<PartialCancelBillPage> createState() => _PartialCancelBillPageState();
}

class _PartialCancelBillPageState extends State<PartialCancelBillPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Partial Cancel Bill", style: TextStyle(color: Colors.white)),
  ...
  Future<Uint8List> _generatePdf(
    ...
    final cancelObj = serverData?['cancel'] ?? {};
    final List cancelledDetails = (cancelObj['room_number'] is List)
        ? List<Map<String, dynamic>>.from(cancelObj['room_number'])
        : [];

    final cancelledRoomCount = cancelledDetails.length;

    String formatCancelledRoom(Map<String, dynamic> r) {
      final name = r['room_name']?.toString() ?? '-';
      final type = r['room_type']?.toString() ?? '-';

      final numsList = (r['cancelled_numbers'] ?? []).map((e) => e.toString()).toList();
      final numsText = numsList.join(', ');
      return "$name ($type) — $numsText ";
    }

    final cancelledRoomsText = cancelledDetails.map((r) => formatCancelledRoom(r)).join('\n');
              ...
              pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    if (cancelledDetails.isNotEmpty)
                      pw.Container(
                        width: double.infinity,
                        color: royalLight,
                        padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 4),
                        margin: const pw.EdgeInsets.only(bottom: 2),
                        child: pw.Text(
                          "CANCELLED ROOM(S) ($cancelledRoomCount)",
                    ...
                    if (serverData!['cancel']['amount_paid'] != null)
                      _cancelLabel("AMOUNT PAID", royalLight, font),

                    if (serverData['cancel']['cancel_charge'] != null)
                      _cancelLabel("CANCELLATION CHARGE", royalLight, font),

                    if (serverData['cancel']['refund'] != null)
                      _cancelLabel("REFUND", royalLight, fontBold),
                  ],
                ),
              ),
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                ...[
                  if (cancelledDetails.isNotEmpty) cancelledRoomsText,
                  if (serverData['cancel']['amount_paid'] != null) "Rs.${serverData['cancel']['amount_paid']}",
                  if (serverData['cancel']['cancel_charge'] != null ) "Rs.${serverData['cancel']['cancel_charge']}",
                  if (serverData['cancel']['refund'] != null) "Rs.${serverData['cancel']['refund']}",
                ].map((amount) {
  ...
  pw.Widget _cancelLabel(String text, PdfColor shade, pw.Font font) {
    return pw.Container(
      width: double.infinity,
      color: shade,
      padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 4),
